...
else console.log('Ticket target not found!');

// 2. Chore Cost Mapping Fixes
...
else console.log('Chore target not found!');
...
for (let i = 0; i < lines.length; i++) {
  if (lines[i].includes('// 5. Animals')) startIdx = i;
  if (startIdx !== -1 && i > startIdx && lines[i].includes('if (hasVipAccess) inventory.hasVip = true;')) {
    endIdx = i;
    break;
  }
}
...
if (startIdx !== -1 && endIdx !== -1) {
  ...
  lines.splice(startIdx, endIdx - startIdx + 1, newAnimalLogic);
  code = lines.join('\n');
} else {
  console.log('Animal block not found! Start:', startIdx, 'End:', endIdx);
}
