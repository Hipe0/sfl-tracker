...
exports.getCropCoins = async (req, res) => {
  try {
...
exports.getFarmData = async (req, res) => {
  ...
  try {
    ...
    const { getTotalBumpkinLevel } = require('../utils/levelCalculator.cjs');

    let hasVipAccess = gameData?.vip?.expiresAt ? (gameData.vip.expiresAt > Date.now()) : false;

    let globalConfig = {
      coinRate: coinRateValue.toString(),
      island: gameData?.island?.type || 'basic',
      tax: null,
      playerName: gameData?.username || `Farm #${farmId}`
    };

    let coinDeliveries = [];

    let bumpkinInfo = { level: null, experience: null, avatar: null };
    if (gameData && gameData.bumpkin) {
      bumpkinInfo.experience = gameData.bumpkin.experience;
      bumpkinInfo.level = getTotalBumpkinLevel(gameData.bumpkin.experience, gameData.island?.ascensionLevel || 0);
    ...
    // Ticket deliveries are now accurately calculated in the parsing loop so no override is needed
      ...
      if (gameData) {
        ...
        if (gameData.desert && gameData.desert.digging) {
          ...
          let streakCollectedAtStr = '';
          if (gameData.desert.digging.streak && gameData.desert.digging.streak.collectedAt) {
            streakCollectedAtStr = new Date(gameData.desert.digging.streak.collectedAt).toISOString().substring(0, 10);
          }
          
          const dugCount = gameData.desert.digging.grid ? gameData.desert.digging.grid.length : 0;
          const status = (streakCollectedAtStr === todayUTCStr) ? 'success' : 'info';
          
          summary.desertDigging = {
            text: `Dug ${dugCount}x | Streak: ${gameData.desert.digging.streak?.count || 0}`,
            status: status
          };
        }
      }
