...
const CROP_SEASONS = {
  ...
  "Potato": ["summer", "autumn", "winter"],
  "Pumpkin": ["autumn"],
  "Carrot": ["spring", "autumn"],
  "Cabbage": ["spring", "winter"],
  "Soybean": ["spring", "autumn"],
  "Beetroot": ["summer", "winter"],
  "Cauliflower": ["summer", "winter"],
  "Parsnip": ["winter"],
  "Eggplant": ["summer"],
  "Corn": ["spring"],
  "Radish": ["summer"],
  "Wheat": ["spring", "summer", "autumn", "winter"],
  "Kale": ["spring", "winter"],
  "Barley": ["spring", "autumn"],
  "Rhubarb": ["spring"],
  "Zucchini": ["summer"],
  "Yam": ["autumn"],
  "Broccoli": ["autumn"],
  ...
  "Turnip": ["winter"],
  "Artichoke": ["autumn"],
  "Tomato": ["spring", "autumn"],
  "Blueberry": ["spring", "winter"],
  "Orange": ["spring", "summer"],
  "Lemon": ["summer", "winter"],
  "Banana": ["summer", "autumn"],
  "Apple": ["autumn", "winter"],
  "Olive": ["spring", "summer", "autumn", "winter"], // Greenhouse crops are available all seasons usually, or I'll just map them. Wait, Rice, Olive, Grape are in all seasons in SEASONAL_SEEDS.
  "Rice": ["spring", "summer", "autumn", "winter"],
  "Grape": ["spring", "summer", "autumn", "winter"]
};
...
export default function MarketTradesPanel() {
  ...
  const currentSeason = farmData?.gameData?.season?.season || 'spring';
  
  const renderSeasonEffect = (season) => {
    const effects = {
      winter: <span className="season-winter-effect text-[14px] inline-block">❄️</span>,
      autumn: <span className="season-autumn-effect text-[14px] inline-block">🍂</span>,
      summer: <span className="season-summer-effect text-[14px] inline-block">☀️</span>,
      spring: <span className="season-spring-effect text-[14px] inline-block">🌸</span>,
    };
    return effects[season] || null;
  };

  const getNextSeason = (season) => {
    const order = ['spring', 'summer', 'autumn', 'winter'];
    const idx = order.indexOf(season);
    return order[(idx + 1) % 4] || 'spring';
  };
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className="flex items-center gap-3">
                            ...
                            <span className="text-slate-400 flex items-center gap-2">
                              {trade.itemName}
                              {CROP_SEASONS[trade.itemName] && (
                                <div className="flex gap-0.5 relative">
                                  {CROP_SEASONS[trade.itemName].map(season => (
                                    <img key={season} src={`/sfl-assets/icons/${season}.webp`} title={season} className="w-3.5 h-3.5 opacity-80 drop-shadow-sm" />
                                  ))}
                                  {CROP_SEASONS[trade.itemName].includes(currentSeason) && (
                                    <div className="absolute -top-3 -right-3 pointer-events-none scale-75">
                                      {renderSeasonEffect(currentSeason)}
                                    </div>
                                  )}
                                </div>
                              )}
              ...
              <div className="p-4 border-b border-slate-700/50 bg-slate-800/30">
                <div className="flex items-center justify-between mb-3">
                  <div className="text-sm font-semibold text-slate-300 flex items-center gap-2">
                    ...
                    <div className="flex items-center gap-1.5 px-2.5 py-0.5 rounded-full bg-slate-800/80 border border-slate-700 shadow-inner overflow-visible">
                      <img src={`/sfl-assets/icons/${currentSeason}.webp`} className="w-4 h-4 object-contain drop-shadow-sm" />
                      <span className="text-[10px] sm:text-xs font-bold text-slate-300 capitalize flex items-center relative">
                        {currentSeason}
                        <div className="absolute -top-2.5 -right-3.5 pointer-events-none">
                          {renderSeasonEffect(currentSeason)}
                        </div>
                      </span>
                    </div>
                        ...
                        <span className="flex flex-col items-start leading-tight">
                          ...
                          {CROP_SEASONS[item] && (
                            <div className="flex gap-0.5 mt-0.5 relative">
                              {CROP_SEASONS[item].map(season => (
                                <img key={season} src={`/sfl-assets/icons/${season}.webp`} title={season} className="w-3 h-3 opacity-80" />
                              ))}
                              {CROP_SEASONS[item].includes(currentSeason) && (
                                <div className="absolute -top-3 -right-4 pointer-events-none scale-75">
                                  {renderSeasonEffect(currentSeason)}
                                </div>
                              )}
                            </div>
                      ...
                      <tr key={g.itemName} className="hover:bg-slate-800/40 transition-colors duration-150">
                          ...
                          <div className="flex items-start gap-4">
                            ...
                            <div>
                              <div className="font-bold text-slate-200 text-base flex items-center gap-2">
                                {g.itemName}
                                {CROP_SEASONS[g.itemName] && (
                                  <div className="flex gap-0.5 ml-1 relative">
                                    {CROP_SEASONS[g.itemName].map(season => (
                                      <img key={season} src={`/sfl-assets/icons/${season}.webp`} title={season} className="w-3.5 h-3.5 opacity-80 drop-shadow-sm" />
                                    ))}
                                    {CROP_SEASONS[g.itemName].includes(currentSeason) && (
                                      <div className="absolute -top-3 -right-4 pointer-events-none scale-75">
                                        {renderSeasonEffect(currentSeason)}
                                      </div>
                                    )}
                        ...
                        <td className="px-4 py-4 align-middle">
                          <div className="flex flex-col items-center justify-center gap-1.5 w-full">
                            ...
                            {/* Next Season Plantable Warning */}
                            {(() => {
                              const nextSeason = getNextSeason(currentSeason);
                              if (CROP_SEASONS[g.itemName] && CROP_SEASONS[g.itemName].includes(nextSeason)) {
                                return (
                                  <div className="mt-2 w-full flex flex-col items-center justify-center p-1.5 rounded-lg border border-indigo-500/30 bg-indigo-500/10 shadow-sm" title={`Có thể trồng vào ${nextSeason}`}>
                                    <span className="text-[9px] font-bold uppercase text-indigo-300 mb-0.5 text-center leading-tight">
                                      Trồng mùa tới
                                    </span>
                                    <img src={`/sfl-assets/icons/${nextSeason}.webp`} className="w-4 h-4 object-contain drop-shadow-sm" />
                                  </div>
                                );
                              }
                              return null;
                      ...
                      <tr key={t.id} className="hover:bg-slate-800/40 transition-colors duration-150">
                        ...
                        <td className="px-6 py-4 font-medium text-slate-200">
                          <div className="flex items-center gap-2">
                            ...
                            {t.itemsStr}
                            {CROP_SEASONS[t.itemName] && (
                              <div className="flex gap-0.5 ml-1 relative">
                                {CROP_SEASONS[t.itemName].map(season => (
                                  <img key={season} src={`/sfl-assets/icons/${season}.webp`} title={season} className="w-3.5 h-3.5 opacity-80 drop-shadow-sm" />
                                ))}
                                {CROP_SEASONS[t.itemName].includes(currentSeason) && (
                                  <div className="absolute -top-3 -right-4 pointer-events-none scale-75">
                                    {renderSeasonEffect(currentSeason)}
                                  </div>
                                )}
