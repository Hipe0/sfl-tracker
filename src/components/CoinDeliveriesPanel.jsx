        ...
        <div className="flex-1 flex flex-col gap-2 justify-start mb-3">
          {(d.reqItems || []).map((item, i) => {
            ...
            return (
              ...
              >
                <div className={`flex items-center gap-2 group relative ${foodRecipes[item.name] || flowerRecipes[item.name] || fishingRecipes[item.name] || fishData[item.name] ? 'cursor-help' : ''}`}>
                  ...
                  <span className={`font-semibold text-[11px] truncate ${foodRecipes[item.name] || flowerRecipes[item.name] || fishingRecipes[item.name] || fishData[item.name] ? 'border-b border-dashed border-emerald-500/50 pb-0.5' : ''}`}>{item.name}</span>
