...
const megastoreItems = [
  { name: 'Moon Hair', cost: '9000', type: 'Shiny Feather', icon: featherIcon, image: '/img/moon_hair.png', buff: '+2 Full Moon Seed stock, +0.5 Full Moon Fruit' },
  { name: 'Astrolabe', cost: '9000', type: 'Shiny Feather', icon: featherIcon, image: '/img/sfts/astrolabe.webp', buff: '15% chance to double Fermentation & Spice Rack output, +5% XP from Aged Fish' },
  { name: 'Cornucopia', cost: '9000', type: 'Shiny Feather', icon: featherIcon, image: '/img/sfts/cornucopia.webp', buff: '+1 Giant Fruit yield from village projects' },
  { name: 'Teamwork Monument', cost: '6000', type: 'Shiny Feather', icon: featherIcon, image: '/img/sfts/teamwork_monument.webp', buff: '+1 Help Limit' },
  { name: 'Ascension Monument', cost: '4000', type: 'Shiny Feather', icon: featherIcon, image: '/img/sfts/ascension_monument.webp', buff: '-20% Expansion Time' },
  { name: 'Otty the Otter', cost: '250', type: 'Otter Pebble', icon: pebbleIcon, image: '/img/sfts/otty_the_otter.webp', buff: '+5 daily fishing reels, +1 random fish every 15th reel' },
...
const mutants = [
  { name: 'Dumbo Octopus', buff: '20% chance of +1 Fish', type: 'Fishing', image: '/img/sfts/dumbo_octopus.webp' },
  { name: 'Ascended Chicken', buff: '+0.1 Egg', type: 'Animal', image: '/img/sfts/ascended_chicken.webp' },
  { name: 'Ascended Sheep', buff: '+0.05 Wool', type: 'Animal', image: '/img/sfts/ascended_sheep.webp' },
  { name: 'Ascended Cow', buff: '-2.5% Cow Sleep Time', type: 'Animal', image: '/img/sfts/ascended_cow.webp' },
  { name: 'Ruins Flower', buff: '+0.05 Honey from full beehives', type: 'Flower', image: '/img/sfts/ruins_flower.webp' }
];

const itemMeta = {
  'Salt Rug': { buff: 'No buff info yet (New item)', image: <div className="w-8 h-8 rounded bg-pink-900/40 border border-pink-500/30 flex shrink-0 items-center justify-center text-pink-400"><img src="data:image/webp;base64," alt="Salt Rug" className="w-6 h-6 object-contain" /></div> },
  'Coat Rack': { buff: 'No buff info yet (New item)', image: <div className="w-8 h-8 rounded bg-stone-900/40 border border-stone-500/30 flex shrink-0 items-center justify-center text-stone-400"><img src="data:image/webp;base64," alt="Coat Rack" className="w-6 h-6 object-contain" /></div> },
  'Rice Shirt': { buff: '-50% Oil to plant Rice, +1 Rice', image: <div className="w-8 h-8 rounded bg-blue-900/40 border border-blue-500/30 flex shrink-0 items-center justify-center text-blue-400"><img src="https://sunflower-land.com/play/wearables/images/413.png" alt="Rice Shirt" className="w-6 h-6 object-contain" /></div> },
  'Vibraphone': { buff: 'Animal feed buffs last 6 harvests', image: <div className="w-8 h-8 rounded bg-amber-900/40 border border-amber-500/30 flex shrink-0 items-center justify-center"><img src="/img/sfts/vibraphone.webp" className="w-6 h-6 object-contain" /></div> },
  'Surfer Hair': { buff: '-50% Salt cost for aging fish', image: <div className="w-8 h-8 rounded bg-yellow-900/40 border border-yellow-500/30 flex shrink-0 items-center justify-center"><img src="https://sunflower-land.com/play/wearables/images/586.png" alt="Surfer Hair" className="w-6 h-6 object-contain" /></div> },
  'Alchemist Apron': { buff: '-50% Potion House fee', image: <div className="w-8 h-8 rounded bg-purple-900/40 border border-purple-500/30 flex shrink-0 items-center justify-center"><img src="https://sunflower-land.com/play/wearables/images/480.png" alt="Alchemist Apron" className="w-6 h-6 object-contain" /></div> },
  'Winged Vase': { buff: '+14% Prime Aged chance', image: <div className="w-8 h-8 rounded bg-sky-900/40 border border-sky-500/30 flex shrink-0 items-center justify-center"><img src="/img/sfts/winged_vase.webp" className="w-6 h-6 object-contain" /></div> },
  'Ascended Idol': { buff: 'Harvest Salt without Salt Rakes', image: <div className="w-8 h-8 rounded bg-emerald-900/40 border border-emerald-500/30 flex shrink-0 items-center justify-center"><img src="/img/sfts/ascended_idol.webp" className="w-6 h-6 object-contain" /></div> },
  'Salt Worker Gnome': { buff: '-30% Salt Node recovery time, +2 Salt per harvest', image: <div className="w-8 h-8 rounded bg-rose-900/40 border border-rose-500/30 flex shrink-0 items-center justify-center"><img src="/img/sfts/salt_worker_gnome.webp" className="w-6 h-6 object-contain" /></div> },
  'Quarry': { buff: 'Mine stone without pickaxes', image: <div className="w-8 h-8 rounded bg-orange-900/40 border border-orange-500/30 flex shrink-0 items-center justify-center"><img src="/img/sfts/quarry.webp" alt="Quarry" className="w-6 h-6 object-contain" /></div> },
  "Autumn's Embrace": { buff: '-50% Crop Time in Autumn', image: <div className="w-8 h-8 rounded bg-amber-900/40 border border-amber-500/30 flex shrink-0 items-center justify-center"><img src="https://sunflower-land.com/play/wearables/images/433.png" alt="Autumn's Embrace" className="w-6 h-6 object-contain" /></div> },
  'Pet': { buff: 'Pet Egg NFT – May receive special pet', image: <div className="w-8 h-8 rounded bg-violet-900/40 border border-violet-500/30 flex shrink-0 items-center justify-center"><img src="/img/icons/pet_nft_egg.png" alt="Pet NFT Egg" className="w-6 h-6 object-contain" /></div> },
  'Tomato Clown': { buff: '-50% Tomato Growth Time', image: <div className="w-8 h-8 rounded bg-red-900/40 border border-red-500/30 flex shrink-0 items-center justify-center"><img src="/img/sfts/tomato_clown.gif" alt="Tomato Clown" className="w-6 h-6 object-contain" /></div> }
};
...
const getPhaseInfo = (timestamp) => {
  ...
  const day = d.getDate();
  if (m === 8 || (m === 9 && day < 10)) return { name: 'Phase 1', res: 'Flowers', icon: flowerIcon, colorClass: 'text-pink-400 bg-pink-900/30 border-pink-500/30' };
  if (m === 9) return { name: 'Phase 2', res: 'Gems', icon: gemIcon, colorClass: 'text-purple-400 bg-purple-900/30 border-purple-500/30' };
    ...
    <div className="flex flex-col gap-6 animate-fade-in-up">
      ...
      <div className="w-full rounded-2xl overflow-hidden shadow-2xl border border-indigo-500/30 relative group">
        ...
        <div className="absolute bottom-4 left-6 z-20">
          ...
          <p className="text-indigo-200 text-sm font-medium">Track Auctions and Megastore Drops</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="glass-panel h-fit">
          ...
          <div className="glass-body p-4 space-y-2">
            {[
              { name: 'Salt Rug',          type: 'Collectible', buff: 'No buff info yet (New item)',              imgSrc: null, imgEl: itemMeta['Salt Rug']?.image },
              { name: 'Coat Rack',         type: 'Collectible', buff: 'No buff info yet (New item)',              imgSrc: null, imgEl: itemMeta['Coat Rack']?.image },
              { name: 'Rice Shirt',        type: 'Wearable',    buff: '-50% Oil to plant Rice, +1 Rice',          imgSrc: 'https://sunflower-land.com/play/wearables/images/413.png' },
              { name: 'Vibraphone',        type: 'Collectible', buff: 'Animal feed buffs last 6 harvests',        imgSrc: '/img/sfts/vibraphone.webp' },
              { name: 'Surfer Hair',       type: 'Wearable',    buff: '-50% Salt cost for aging fish',            imgSrc: 'https://sunflower-land.com/play/wearables/images/586.png' },
              { name: 'Alchemist Apron',   type: 'Wearable',    buff: '-50% Potion House fee',                    imgSrc: 'https://sunflower-land.com/play/wearables/images/480.png' },
              { name: 'Winged Vase',       type: 'Collectible', buff: '+14% Prime Aged chance',                   imgSrc: '/img/sfts/winged_vase.webp' },
              { name: 'Ascended Idol',     type: 'Collectible', buff: 'Harvest Salt without Salt Rakes',          imgSrc: '/img/sfts/ascended_idol.webp' },
              { name: 'Salt Worker Gnome', type: 'Collectible', buff: '-30% Salt Node recovery time, +2 Salt per harvest', imgSrc: '/img/sfts/salt_worker_gnome.webp' },
              { name: 'Quarry',            type: 'Collectible', buff: 'Mine stone without pickaxes',              imgSrc: '/img/sfts/quarry.webp' },
              { name: "Autumn's Embrace",  type: 'Wearable',    buff: '-50% Crop Time in Autumn',                  imgSrc: 'https://sunflower-land.com/play/wearables/images/433.png' },
              { name: 'Tomato Clown',      type: 'Collectible', buff: '-50% Tomato Growth Time',                  imgSrc: '/img/sfts/tomato_clown.gif' },
              { name: 'Pet',               type: 'NFT',         buff: 'Pet Egg NFT – May receive special pet',    imgSrc: '/img/icons/pet_nft_egg.png' },
            ].map((item, idx) => {
              const typeColors = {
            ...
            {megastoreItems.map((item, idx) => (
              <div key={idx} className="bg-slate-800/60 p-2.5 rounded-xl border border-slate-700/50 shadow-sm flex items-center gap-3 hover:border-pink-500/30 transition-colors group">
                ...
                <div className="flex flex-col items-end gap-1 shrink-0">
                  ...
                  {item.hasVipDiscount && <span className="px-1.5 py-0.5 bg-amber-500/20 text-amber-400 border border-amber-500/50 rounded text-[9px] uppercase font-bold" title="50% off with VIP Banner">VIP -50%</span>}
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="glass-panel">
        <div className="glass-header bg-gradient-to-r from-amber-900/40 to-orange-900/40 border-b border-amber-500/30 flex justify-between items-center">
          <span className="flex items-center text-amber-400">
            <i className="bi bi-calendar2-week mr-2 text-xl"></i> Auction Details
          </span>
          <span className="text-xs text-amber-200/70 bg-black/20 px-2 py-1 rounded border border-amber-900/30">Displayed in GMT+7</span>
        </div>
                          ...
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              <div className="font-bold text-slate-200 flex items-center gap-2">
                                {item.name}
                                {isNow && <span className="bg-emerald-500 text-white text-[9px] px-1.5 py-0.5 rounded uppercase font-black tracking-wider animate-bounce">Active Now</span>}
                              </div>
